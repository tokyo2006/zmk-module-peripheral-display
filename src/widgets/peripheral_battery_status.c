...
static void set_row(lv_obj_t *label, lv_obj_t *icon, uint8_t pct) {
    if (pct > 100) pct = 100;
    ...
    const struct peripheral_status_adv_data *s)
{
    ...
    w->state = new_state;
    set_row(central_label,    central_icon,    new_state.central_pct);
    set_row(peripheral_label, peripheral_icon, new_state.peripheral_pct);
}
