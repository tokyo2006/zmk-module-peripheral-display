...
/* sym_battery's interior (inside the 1px border) is 6x4 px starting at
 * (1,1). Size the fill bar to that fraction of pct so the icon actually
 * reflects charge instead of always drawing empty. */
static void set_battery_fill(lv_obj_t *fill, lv_obj_t *icon, uint8_t pct) {
    int w = (pct * 6) / 100;
    if (pct > 0 && w < 1) w = 1;
    lv_obj_set_width(fill, w);
    lv_obj_align_to(fill, icon, LV_ALIGN_TOP_LEFT, 1, 1);
}
